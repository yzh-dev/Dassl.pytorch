...
class LocNet(nn.Module):
    ...
    def forward(self, x):  # batch*3*224*224
        x = self.backbone(x)  # batch*64*14*14
        x = x.view(x.size(0), -1)
        x = self.fc_loc(x)  # batch*4
        x = torch.tanh(x)
        x = x.view(-1, 2, 2)  # batch*2*2
        theta = x.data.new_zeros(x.size(0), 2, 3)  # # batch*2*3
        theta[:, :, :2] = x
        return theta  # 返回仿射变换矩阵所需参数
...
class FCN(nn.Module):
    ...
    def stn(self, x):
        ...
        theta = self.locnet(x)  # Localisation Network-局部网络，决定输入所需变换的参数theta, shape:[2,3]
        # 此步骤的目地是为了得到输出特征图的坐标点对应的输入特征图的坐标点的位置
        # https://zhuanlan.zhihu.com/p/91482941
        # https://zhuanlan.zhihu.com/p/41738716
        grid = F.affine_grid(theta, x.size())  # Parameterised Sampling Grid-参数化网格采样  batch*224*224*2
        return F.grid_sample(x, grid), theta  # Differentiable Image Sampling-差分图像采样

    def forward(self, x, lmda=1.0, return_p=False, return_stn_output=False):  # BCHW
        """
        Args:
        ...
        theta = None
        if self.locnet is not None:
            x, theta = self.stn(x)  # 经过空间变换网络stn不改变shape
        input = x  # batch*3*224*224

        x = self.backbone(x)  # DTN主干网络（多个ResBlock），batch*64*224*224
        if self.gctx_fusion is not None:
            c = F.adaptive_avg_pool2d(x, (1, 1))  # batch*64*1*1
            c = c.expand_as(x)  # batch*64*224*224
            x = torch.cat([x, c], 1)  # batch*128*224*224
            x = self.gctx_fusion(x)  # batch*64*224*224
